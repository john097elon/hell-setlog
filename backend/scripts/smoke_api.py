...
def main() -> None:
    ...
    status, user = request("POST", "/api/auth/register", {
        "username": USERNAME,
        "email": EMAIL,
        "password": PASSWORD,
    })
    ...
    status, login = request("POST", "/api/auth/login", {"username": USERNAME, "password": PASSWORD})
    ...
    status, party = request("POST", "/api/parties/", {"name": "Smoke Party"}, token=token)
    expect(status, 201, "create party")

    status, workout = request("POST", "/api/workouts/", {"party_id": party["id"], "notes": "smoke"}, token=token)
    expect(status, 201, "create workout")

    print("Smoke API flow passed:", json.dumps({
        "user_id": user["id"],
        "party_id": party["id"],
