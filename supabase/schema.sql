...
-- profiles 참조는 PostgREST가 멤버와 프로필을 한 번에 읽게 해 준다. 빼면 멤버 목록과
-- 주간 미션 질의가 관계를 찾지 못해 실패한다.
create table if not exists party_members (party_id uuid not null references parties(id) on delete cascade,user_id uuid not null references auth.users(id) on delete cascade,role text not null default 'member',joined_at timestamptz not null default now(),primary key(party_id,user_id),constraint party_members_user_id_profile_fkey foreign key (user_id) references public.profiles(user_id) on delete cascade);
