...
- Folder layout: `core/` contains theme/router/error/extensions/DI; `data/` contains local/remote/repositories; `domain/` contains entities/repositories/usecases; `features/` contains workout_log, routine, stats, timer, exercise_db, party, feed, and character with presentation/application.
- Core entities: Exercise(id, name, nameKo, muscleGroup, equipment, isCustom, thumbnailUrl); Routine(id, name, description, ownerId, isTemplate, createdAt); RoutineItem(id, routineId, exerciseId, order, targetSets, targetReps, targetWeight); WorkoutSession(id, userId, routineId?, partyId?, startedAt, endedAt, memo, totalVolume); WorkoutSet(id, sessionId, exerciseId, setIndex, weight, reps, rpe?, isWarmup, isCompleted, restSeconds, completedAt); PersonalRecord(id, userId, exerciseId, type, value, achievedAt, sessionId); Party(id, name, description, ownerId, inviteCode, memberLimit, createdAt); PartyMember(partyId, userId, role, joinedAt, streakDays); PartyPost(id, partyId, userId, sessionId?, type, body, createdAt); VideoClip(id, userId, setId?, sessionId, storagePath, thumbPath, durationMs, visibility, createdAt); Character(userId, level, exp, stage, appearanceJson, updatedAt); CharacterStat(userId, statType, value); ExpEvent(id, userId, source, amount, createdAt).
...
- Workout UX: prefill the prior set's weight and reps, complete a set in one tap, begin rest automatically, and offer swipe-delete undo without a confirmation dialog.
```
